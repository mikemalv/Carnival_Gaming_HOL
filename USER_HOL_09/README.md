# Carnival Cruise Gaming HOL -- User 09
...
| Setting | Value |
|---------|-------|
| Role | `HOL_USER_09_ROLE` |
| Warehouse | `HOL_USER_09_WH` |
| Database | `HOL_USER_09_DB` |
...
2. Open **HOL_WORKSPACES > USER_HOL_09**
...
3. **Upload the semantic model file:**
   - In Snowsight, navigate to: **Data > HOL_USER_09_DB > GOLD > Stages > SEMANTIC_MODELS**
...
5. **Set up CoWork:**
   ...
   - Warehouse: `HOL_USER_09_WH`
   - Add your semantic view: `HOL_USER_09_DB.GOLD.GAMING_SEMANTIC_MODEL`
   - Click **Create**
...
| "Insufficient privileges" | Run `SELECT CURRENT_ROLE();` -- you should see `HOL_USER_09_ROLE`. If not, run `USE ROLE HOL_USER_09_ROLE;` |
| Dynamic Table not refreshing | Run `SHOW DYNAMIC TABLES IN DATABASE HOL_USER_09_DB;` and check the `SCHEDULING_STATE` column |
...
| AI function errors | Ask your instructor to verify the `SNOWFLAKE.CORTEX_USER` database role is granted to `HOL_USER_09_ROLE` |
| Wrong results in queries | Check that you are in the right database: `SELECT CURRENT_DATABASE();` should show `HOL_USER_09_DB` |
