...
--   ALTER WORKSPACE HOL_WORKSPACES.PUBLIC.USER_HOL_03 COMMIT;
-- to publish your change, then re-run this COPY FILES.
-- ==========================================================================

COPY FILES INTO @GOLD.SEMANTIC_MODELS/
FROM 'snow://workspace/HOL_WORKSPACES.PUBLIC.USER_HOL_03/versions/head/'
...
BEGIN
  ...
  CALL SYSTEM$CREATE_SEMANTIC_VIEW_FROM_YAML('HOL_USER_03_DB.GOLD', :yaml_str);
END;
...
CREATE OR REPLACE AGENT GOLD.CASINO_ANALYST
  ...
  PROFILE = '{"display_name": "Casino Analytics - User 03"}'
  FROM SPECIFICATION
$$
models:
  orchestration: auto
...
tool_resources:
  Casino_Metrics:
    semantic_view: "HOL_USER_03_DB.GOLD.GAMING_SEMANTIC_MODEL"
  Review_Search:
    search_service: "HOL_USER_03_DB.GOLD.REVIEW_SEARCH"
    max_results: "8"
    id_column: "review_id"
$$;
...
BEGIN
  ALTER SNOWFLAKE INTELLIGENCE SNOWFLAKE_INTELLIGENCE_OBJECT_DEFAULT
    ADD AGENT HOL_USER_03_DB.GOLD.CASINO_ANALYST;
...
-- 2. Select "Casino Analytics - User 03" from the agent list.
