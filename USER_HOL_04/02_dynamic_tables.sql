...
USE ROLE HOL_USER_04_ROLE;
USE WAREHOUSE HOL_USER_04_WH;
USE DATABASE HOL_USER_04_DB;

-- ==========================================================================
-- SILVER LAYER: Join and enrich raw data
-- ==========================================================================

-- Dynamic Table 1: Enriched gaming transactions
-- Joins transactions with ship, game, and player details
CREATE OR REPLACE DYNAMIC TABLE SILVER.GAMING_TRANSACTIONS_H
  TARGET_LAG = '1 minute'
...
FROM BRONZE.GAMING_TRANSACTIONS t
JOIN BRONZE.PLAYERS p ON t.player_id = p.player_id
JOIN BRONZE.SHIPS s ON t.ship_id = s.ship_id
JOIN BRONZE.CASINO_GAMES g ON t.game_id = g.game_id
JOIN BRONZE.VOYAGES v ON t.voyage_id = v.voyage_id;

-- Dynamic Table 2: Player summary roll-up
CREATE OR REPLACE DYNAMIC TABLE SILVER.PLAYER_SUMMARY_H
  TARGET_LAG = '1 minute'
  WAREHOUSE = HOL_USER_04_WH
AS
SELECT
...
CREATE OR REPLACE DYNAMIC TABLE GOLD.DAILY_SHIP_REVENUE
  TARGET_LAG = '1 minute'
  WAREHOUSE = HOL_USER_04_WH
AS
SELECT
...
SHOW DYNAMIC TABLES IN DATABASE HOL_USER_04_DB;
